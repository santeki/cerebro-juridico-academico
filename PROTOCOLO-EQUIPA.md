...
Pipeline dentro da sessão (mandato de 2026-07-26): enquanto o maestro lê o bloco N, os despachos preparam do bloco N+1 apenas o estritamente mecânico - confronto MD↔PDF-texto, renders, contagens -, e os diffs de propagação preparam-se em paralelo com a triagem dos achados. Candidatos de extracção interpretativos só se despacham depois de o maestro ter lido o bloco respectivo: a leitura integral do maestro nunca recebe agenda feita. A ronda E2 mantém-se única por sessão (cadência, acima). Invariantes intocadas: sub-agentes lêem e reportam, nunca escrevem; todo o achado é verificado contra a fonte antes de aplicar.

## Atribuição de modelos aos papéis

Decisão do aluno na conversa 19 (mandato expresso de 2026-07-25), na sequência da revisão total dirigida às sessões 3-5: os papéis repartem-se por dois modelos, com fronteira funcional e revisão cruzada. A repartição funda-se nos achados dessa revisão - fidelidade de extracção do executor excelente (zero invenções em 750 confrontos), com o padrão de erro na síntese ambiciosa e nas marcas; registo do orquestrador a subordinar a síntese ao verificável, carecendo igualmente de revisor -, não em juízo de marca.
...
- **Fallback e validação.** O override valida-se no primeiro despacho de cada conversa - o primeiro acto pede ao sub-agente a identificação do modelo em que corre e o resultado regista-se no `log.md` da conversa. Onde o override não esteja disponível no ambiente, o despacho corre no modelo da sessão, com o modelo efectivo registado; a separação escritor/revisor mantém-se por passagens separadas com escopos cegos (o regime sem dispatch da revisão dupla, acima).
