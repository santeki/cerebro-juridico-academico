...
2. Integrar a afirmação na página, com wikilink para a página de fonte e a âncora de localização. Cada frase substantiva fica rastreável (regra 4 do `CLAUDE.md`). Em página de conceito, a afirmação doutrinária integra-se na secção *Posição de [Autor]* respectiva e a *Noção* da página revê-se no mesmo passo (`modelos/modelo-conceito.md`).
3. Quando duas fontes convergem sobre o mesmo ponto, identificar ambas. Quando coexistem sobre o mesmo conceito — convirjam ou divirjam —, disparar o **Eixo B** (`PROTOCOLO-ANALISE-COMPARATIVA.md`): classificar a relação (`relacao:` — complementares, compatíveis, distintas, em-conflito), sub-classificar se em conflito, e percorrer os Comparativos. A divergência viva regista-se, por defeito, na secção *Relação entre as posições* da página do conceito; sobe a página própria em `wiki/Debates/` quando atravesse mais de um conceito (sede única contra a duplicação), quando envolva três ou mais vozes com posições articuladas, ou por decisão expressa do aluno — e a página de Debate remete às páginas de conceito, que devolvem a remissão.
...
Quando a fonte tem capítulos e centenas de páginas, a leitura integral organiza-se sem perder fôlego nem supervisão: **agenda de leitura** no arranque — as perguntas que a wiki actual leva à fonte e as hipóteses sobre a posição do autor que se querem testar, registadas na página de fonte antes da primeira página lida (é o que torna a leitura activa); **bloco panorâmico** único — pré-leitura do índice de todos os capítulos, com função expectável e conceitos prováveis por capítulo (dispensável em obra curta ou uniforme, com nota); **refresher** no início de cada capítulo — reler a entrada panorâmica desse capítulo e fixar as páginas a tocar; **pausa entre capítulos** — cada capítulo fecha com a extracção feita e as páginas actualizadas, e a continuação propõe-se numa linha com default marcado, ficando as decisões substantivas (categoria nova, conflito com página existente) sempre à espera de quem opera; e, no fecho da obra, **releitura selectiva** dos capítulos cujo sentido mudou à luz dos posteriores — a iluminação retroactiva regista-se na página; obra linear dispensa, com nota. O campo `progresso:` do frontmatter diz sempre onde a leitura vai. As fronteiras de cada recorte de sessão fixam-se pelos **cabeçalhos impressos do exemplar**, nunca só pelo índice da obra: antes de cortar, confirmam-se a página de abertura da unidade seguinte e a de fecho da unidade em leitura pelos cabeçalhos corridos (ou pelo separador de parte), o recorte leva margem para lá da fronteira esperada, e a divergência entre índice e cabeçalhos regista-se no mapa de tratamento — o índice é mapa provisório, não fronteira (padrão observado cinco vezes consecutivas no mesmo exemplar, sessões 2 a 6; registo na página da fonte e no log). Quando a página de fonte exceda o orçamento das CONVENCOES, os registos de sessão vivem em **sub-páginas de leitura** («<obra> — leitura da sessão N»), com a página-mãe a manter identidade, agenda, mapa de tratamento, dossiers transversais e o índice das sessões; excedido o orçamento uma vez, o regime de sub-páginas mantém-se para todas as sessões seguintes, e o registo novo de cada sessão escreve-se directamente na sua sub-página; a sub-página herda o orçamento como limite mole — o registo de uma sessão é unidade indivisível e não se fragmenta para o cumprir (excedente adjudicado, com nota).

O fecho da obra longa corre, por esta ordem e com pausa entre passos: releitura selectiva (iluminação retroactiva registada na página de fonte; «Tese central» preenchida); revisão total — auditoria integral dos quatro planos sobre todo o cofre e conferência de coerência de todos os derivados (index, log, vistas com a «Cobertura do programa», PAINEL, ESTADO-RESUMO, telemetria e doc de estado do projecto); e, havendo conversão externa do exemplar, a entrega consolidada — errata completa e relatório de padrões regenerado — preparada uma só vez, no fim (Decisões de 2026-07-10). Cada passo do fecho que altere o estado do cofre regenera todos os derivados de leitura à escala — index, PAINEL, ESTADO-RESUMO — e não apenas o painel: derivado não regenerado é retrato desactualizado que a conversa seguinte lê como actual. A unidade da sessão de leitura é o **capítulo** — ou bloco coerente de dimensão equivalente, em regra até ≈ 50 páginas —, nunca mais do que isso sem decisão expressa do aluno: a sessão curta com ritual completo bate a sessão longa com ritual comprimido (mandato expresso de 2026-07-18).
...
## Obra convertida — disciplina da dupla fonte na sessão

Quando o exemplar em `raw/` é conversão (MD derivado de PDF), o MD é instrumento de leitura e o PDF é a verdade — e a sessão de leitura observa quatro regras endurecidas pela prática (conversas 19-20): (1) **em zona fraca declarada** na página de fonte ou na sub-página de leitura respectiva (margem cortada, sublinhados densos, degradação física), citação literal só com adjudicação por imagem — a convergência de derivados corrompidos pela mesma causa não é convergência; (2) **contagens e inventários por código**, nunca por leitura corrida — folhas de um recorte, citações novas, entradas de errata e wikilinks contam-se mecanicamente, e as referências numéricas dos gates fixam-se **depois** da regeneração dos derivados, nunca antes; (3) **substituições no texto do cofre com correspondência exacta verificada** — localizar o trecho por pesquisa literal antes de substituir e confirmar a aplicação depois: substituição silenciosa que não encontra o alvo é falha que passa despercebida; (4) na sondagem ao PDF que não bata certo, **confirmar primeiro o mapeamento folha↔página pelo cabeçalho impresso** (o probe da secção «Paginação de PDF», abaixo) antes de diagnosticar divergência — a confusão folha/página gera falsos alarmes que consomem adjudicação.
