...
### Passe mecânico de âncoras (por código, não por geração)

Sub-passe deste plano, executado por script e não por juízo do modelo - é o que quebra a circularidade de o agente se avaliar a si próprio: (a) cada citação «...» com âncora a `raw/` confere-se contra o ficheiro e a página indicados - o trecho existe ou não existe; (b) cada wikilink resolve para página existente; (c) cada atribuição N1 tem âncora presente; (d) cada «em relato» identifica mediador com âncora. Saída: relatório-semáforo, legível numa linha - páginas varridas, âncoras confirmadas, quebradas, órfãs. O que o passe não cobre, e diz-se sempre: a fidelidade semântica - a paráfrase que cita a página certa e distorce o sentido - não é mecanizável; essa fatia cobre-se pelo selo de revisão da origem (`revisto_pelo_aluno`) e pelos *spot-checks* do fecho de ingestão (`PROTOCOLO-INGESTAO.md`). Em cópias distribuídas, os achados deste plano alimentam a errata da edição (`PROTOCOLO-INGESTAO.md`). O passe confere ainda o `hash_raw` de cada página de fonte contra o exemplar em `raw/` - divergência é adulteração ou troca de exemplar, e trava como qualquer âncora quebrada. Em exemplar convertido com errata própria, a conferência da alínea (a) admite três níveis, todos mecânicos: correspondência exacta no derivado de leitura (MD); correspondência após aplicação das correcções da errata; correspondência por padrão documentado de conversão (lista fechada no relatório de padrões do exemplar). O terceiro nível implementa-se por canonização das decorações, nunca por dobra de palavras: canonizam-se ordinal, aspas, ênfase, pontuação-ruído, chamada de nota e translineação; os dígitos não se tocam em nenhum passo, porque é neles que vive o risco jurídico - número de artigo, de n.º, de alínea, de página -, e nenhuma dobra de forma de letra entra sem constar da lista fechada do relatório do exemplar. A camada só vale depois de passar controlo positivo (as citações já cobertas continuam cobertas, sem uma única regressão) e controlo negativo (adulterações injectadas continuam a falhar, e a camada não é mais cega do que o nível que substitui). Os dois controlos correm-se e registam-se sempre que a camada mude. Limite declarado da camada: o recorte que inverte o sentido sem alterar uma letra (supressão de «não» inicial e afins) é fidelidade semântica, não mecanizável, e a sua cobertura continua a ser dos *spot-checks* e da revisão humana (6.26, item A). Citações que, embora ancoradas em frase da fonte convertida, transcrevam fonte primária própria ou gralha do exemplar marcada [sic] verificam-se contra a sua fonte e entram numa lista fora-de-escopo justificada. Uma falha bloqueia.

Ao passe estrutural e ao verificador global acresce, no fecho de cada conversa, o **crivo de citações**, com os checks C1 (cobertura), C3 (marcador de trabalho por concluir) e C4 (aspas equilibradas) bloqueantes, e o C2 informativo enquanto não passar os dois controlos. Uma falha bloqueia o fecho. Condição de entrada em vigor do gate: só se arma depois de a camada de nível 3 estar instalada com os dois controlos registados e de a fila de erratas das citações exactas estar vazia ou explicitamente diferida com prazo - armar um gate sobre um instrumento que reporta falhas falsas é ensinar a ignorá-lo (6.26, item D).

### Passe estrutural do esqueleto (por código)

Ao lado do passe de âncoras, e com a mesma natureza - script, não juízo -, verifica-se a integridade do próprio sistema: cada remissão inter-ficheiros (`X.md` em texto normativo) resolve para ficheiro existente - os registos históricos datados e os exemplos marcados como tal ficam de fora, porque citam legitimamente o estado da época ou ilustram o padrão; a árvore documentada nas convenções corresponde ao disco, nos dois sentidos; cada tipo do enum tem o seu modelo (fora os singulares de máquina) e cada modelo o seu tipo; cada caminho da tabela de ponteiros existe; e os exemplos seguem a nomenclatura natural - o exemplo ensina o padrão, tem de o cumprir. Verifica ainda, por código, o frontmatter mínimo de cada página de `wiki/` (tipo pertencente ao enum das `CONVENCOES.md`; `titulo` presente) e a cobertura índice↔wiki nos dois sentidos (página de conhecimento sem linha no `index.md`; linha do índice sem página no disco) - a fatia mecanizável da coerência índice↔wiki↔log enunciada acima passa a conferência por script; a coerência com o `log.md` mantém-se verificação de leitura. Wikilink quebrado ou remissão órfã é falha, não ruído. Corre na auditoria integral e, obrigatoriamente, na verificação final do empacotamento (`PROTOCOLO-EMPACOTAMENTO.md`). As exclusões praticadas ficam codificadas: os exemplos marcados («Ex.:», «p. ex.») das CONVENCOES; a menção em prosa a `log.md` no § Versionamento do `CLAUDE.md`; o `[[página]]` paramétrico de `telemetria.md`; a história datada de `log.md` e do `CHANGELOG.md`; os nomes paramétricos (`X.md`) e os ficheiros gerados a pedido (`ERRATA-EDICAO.md`); e os caminhos com prefixo `claude/` (docs de projecto claude.ai - proveniência externa ao vault, não remissão interna). Desde a 6.22, o crivo confere ainda a **frescura dos derivados** - os contadores do `ESTADO-RESUMO.md` (páginas por categoria, comparativas, aguarda-comparativo, entradas de ERRATA-MD) e o cabeçalho do dossier de erratas da página-mãe do Ascensão - a única com cabeçalho de contagem - contra a verdade mecânica (disco, frontmatters, ficheiros de errata; checks 9 e 10 do `crivo_estrutural.py` v3, em `projecto/ferramentas/`); desalinhamento é falha, e mudança de formato do derivado que quebre o padrão do contrato é falha declarada, não silêncio - o derivado mantém o formato ou o contrato actualiza-se com ele. E relata, como informação sem bloqueio, as páginas de conceito que citam norma sem o carimbo `vigencia_conferida:` no frontmatter (formato do carimbo, quando presente, é validado - malformado é falha); o relatório informativo converte-se em gate quando a camada de vigência estiver instalada (bloco de vigência). Confere também a disciplina de escrita das `CONVENCOES.md`, e a falha bloqueia: o travessão «—» fora de citação literal, de código ou de wikilink, e o parágrafo que abre com rótulo a negrito seguido de ponto ou de dois pontos. O bloco `>` não isenta por si, porque é por ele que a prosa em callout escapava; a citação longa em bloco preserva-se pelas aspas curvas que a house style exige. O escopo dos dois checks exclui `raw/`, arquivo dos derivados das obras, e `ferramentas/`, cujos `.md` são relatórios de instrumento gerados por código (6.27).
...
### Passe do grafo por código (por script, não por juízo)

As três verificações de teia do plano - órfãs, assimetrias e dimensionamento - correm mecanicamente por `grafo_semantico.py` (`projecto/ferramentas/`, ao lado do crivo e do verificador): o script constrói o grafo dirigido dos wikilinks entre páginas de conhecimento e relata (a) páginas de conhecimento sem qualquer link de entrada vindo de outra página de conhecimento ou de uma vista de faculdade (órfãs - a entrada de vista, mapeamento curricular selectivo, afasta a orfandade sem entrar no grafo de assimetrias; as entradas de catálogo não a afastam); (b) pares A→B sem retorno B→A (assimetrias); (c) páginas de conceito ou instituto com menos de duas ligações totais, como informação de dimensionamento, sem limiar normativo. Ligações de navegação não contam para o grafo: `index.md`, `log.md`, `PAINEL.md`, `ESTADO-RESUMO.md`, `telemetria.md`, `MELHORIA.md`, `GLOSSARIO.md`, `SUPRESSAO-LIST.md`, `GATILHOS.md`, `VOZ-EVOLUCAO.md`, `VOZ-FINGERPRINT.md`, `Faculdades.md` e as vistas de faculdade entram como emissores e receptores de navegação, não como teia de conhecimento; as fontes de avaliação ficam igualmente fora do conjunto. A saída é relatório datado de achados para adjudicação - o passe nunca escreve nas páginas, nunca cria recíprocos automáticos (a reciprocidade é «em regra», não universal: a adjudicação distingue a assimetria-lacuna da assimetria justificada, como as seis do mapa de sessões futuras adjudicadas a 2026-07-19), e um achado seu não bloqueia fecho por si - é achado de Plano 3, com prioridade de triagem normal. Desde a 6.22, as arestas com origem ou destino em sub-página - identificada pelo padrão de nome: «— leitura da sessão N», «— leitura do capítulo R», «— posição de {Autor}» - **dobram-se na página-mãe** para o cômputo das assimetrias - a aresta da sub-página tem o retorno na mãe, e contá-la solta inundaria o relatório com assimetrias estruturais da cisão; o relatório indica o total bruto ao lado do dobrado. Corre na auditoria integral e, como informação, no fecho de cada sessão de leitura.
