...
- [2026-07-11] auditoria | **Revisão total do fecho do BM (passo 2)** — auditoria integral 4 planos + coerência dos derivados. **Verificador global reconstruído e alargado** (varre também as frases BM-ancoradas das páginas): VERDE — 942 citações (764 exactas no MD; 93 por errata; 75 por padrão documentado de conversão; 10 fora de escopo com fonte primária/[sic] documentados; 0 falhas); hash_raw ✓. Achados corrigidos: **10 infidelidades de citação** nas páginas (adaptações de flexão/condensações — letra da fonte reposta); **22 assimetrias de cross-linking** fechadas com retornos em 14 páginas; **erratas «em atraso» 155-186** (+ reparação das células assimétricas 82/93, que corrompiam a aplicação mecânica, e afinações 123/145/162/176/184; total 186); index↔wiki↔log coerentes; sem órfãs; Eixo B sem pendências (corpo monocamada). MELHORIA.md inaugurado (passagem 1 + tracker). Releitura selectiva e «Tese central» fechadas no passo 1 (secção própria na fonte).
