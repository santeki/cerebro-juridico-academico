...
_Para construir: reunir amostras de escrita do utilizador (sebentas, resumos, ensaios, respostas) e correr «constrói a voz». O fingerprint resultante substitui este placeholder, com `corpo_amostras` a identificar os textos lidos e `estado_verificacao` a subir de `em-aberto` para `parcial` ou `verificado` consoante a cobertura._
