---
tipo: instituto
titulo: "{Instituto jurídico}"
categoria: [Instituto]              # classificação(ões) que as fontes dão; plural, mutável
ramo: []
estado_verificacao: verificado | parcial | em-aberto
relacao:                            # quando ≥ 2 fontes tratam o instituto (Eixo B)
...
### Noção
{Definição e delimitação negativa (o que NÃO é). Distinguir definição legal de doutrinária. Cada afirmação ancorada.}
...
### Natureza jurídica
{Qualificação dogmática. Se controvertida, abrir o mapa e remeter para [[Debates/...]]; marcar se a caracterização vem de doutrina especializada ou de divulgação.}

### Requisitos / pressupostos
{Enumeração nua (a), (b), (c). Não fechar a lista além do que as fontes fecham. Cada requisito ancorado.}
...
### Prazos relevantes
{Caducidade, prescrição, prazos processuais associados.}

### Pontos de atenção
{Armadilhas recorrentes, particularidades, casos-limite.}
...
## Reclassificações
{Quando uma fonte nova altera a balança de uma posição dominante: data, fonte que disparou, fundamentação curta. O texto antigo migra para aqui; o novo entra na construção dogmática principal.}
